  ...
  const handleDownloadPDF = async (session: Session) => {
    try {
      ...
      if (session.frames && session.frames.length > 0) {
        yPosition += 20;
        doc.text(`Number of captures: ${session.frames.length}`, 20, yPosition);
      }
    ...
    <div className="space-y-6">
      {sessions.map((session) => (
        <div key={session._id} className="bg-white p-6 rounded-lg border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,0.8)]">
          ...
          {expandedSession === session._id && session.report && (
            <div className="mt-6 bg-blue-50 p-6 rounded-lg border-2 border-black">
